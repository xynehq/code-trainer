...
class EnhancedPythonDatasetPreparator:
    ...
    Also processes markdown documentation files for comprehensive understanding
    """
    ...
    def clean_markdown_content(self, content: str) -> str:
        """Clean markdown content by removing images and other non-useful elements"""
    ...
    def extract_quick_metadata(self, content: str) -> Dict:
        ...
        try:
            ...
            # Fallback in case of invalid Python syntax
            return {
                'functions': 0,
                'classes': 0,
                'methods': 0,
    ...
    def extract_functions(self, content: str, file_path: str, module: str) -> List[Dict]:
...
"""
    ...
    def extract_methods(self, content: str, file_path: str, module: str) -> List[Dict]:
...
"""
    ...
    def extract_classes(self, content: str, file_path: str, module: str) -> List[Dict]:
    ...
    def extract_module_structure(self, content: str, file_path: str, module: str) -> List[Dict]:
    ...
    def extract_ast_information(self, content: str, file_path: str, module: str) -> List[Dict]:
...
"""
    ...
    def collect_markdown_files(self) -> List[Path]:
        """Collect relevant Markdown files from repository"""
        logger.info(f"Collecting Markdown files from {self.repo_path}")
        markdown_files = []
        
        # Common markdown extensions
        for pattern in ["*.md", "*.markdown", "*.mdown", "*.mkd"]:
            markdown_files.extend(self.repo_path.rglob(pattern))
        ...
        for f in markdown_files:
            if self.is_excluded_path(f):
                continue
            if self.is_relevant_markdown(f):
                relevant_files.append(f)
        
        logger.info(f"✓ Found {len(relevant_files)} relevant Markdown files")
        return relevant_files
    
    def is_relevant_markdown(self, file_path: Path) -> bool:
        """Check if markdown file is relevant for the dataset"""
    ...
    def process_markdown_file(self, file_path: Path) -> List[Dict]:
        """Process a single markdown file and extract content with chunking"""
        try:
            content = file_path.read_text(encoding='utf-8', errors='ignore')
            rel_path = file_path.relative_to(self.repo_path)
            
            # Clean markdown content and filter out images
...
"""
            ...
            return self.chunk_markdown_content(full_content, str(rel_path))
            
        except Exception as e:
            logger.debug(f"Error processing markdown file {file_path}: {e}")
            return []
    
    def chunk_markdown_content(self, content: str, file_path: str) -> List[Dict]:
        """Split markdown content into chunks with overlap if needed"""
    ...
    def process_file(self, file_path: Path) -> Tuple[List[Dict], List[Dict]]:
        ...
        try:
            ...
            # Extract granular samples
            granular_samples = []
            
            if not is_test and (metadata['functions'] > 0 or metadata['classes'] > 0):
                granular_samples.extend(
                    self.extract_functions(content, str(rel_path), module)
                )
                
                granular_samples.extend(
                    self.extract_classes(content, str(rel_path), module)
                )
                
                granular_samples.extend(
                    self.extract_methods(content, str(rel_path), module)
                )
                
                granular_samples.extend(
                    self.extract_module_structure(content, str(rel_path), module)
                )
            
            # Always extract AST information for non-test files (regardless of functions/classes)
            if not is_test:
                granular_samples.extend(
                    self.extract_ast_information(content, str(rel_path), module)
                )
    ...
    def process_all_files(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        logger.info("Processing Markdown files for documentation data...")
        for file_path in tqdm(markdown_files, desc="Processing Markdown", unit="files"):
            markdown_samples = self.process_markdown_file(file_path)
            all_file_samples.extend(markdown_samples)  # Markdown samples go into file_samples
        
        logger.info(f"✓ Created {len(all_file_samples)} file-level samples (including markdown)")
    ...
    def save_dataset(self, file_samples: List[Dict], granular_samples: List[Dict]):
        ...
        stats['documentation_breakdown'] = {
            'markdown_files': stats['sample_types'].get('markdown_file', 0),
            'markdown_chunks': stats['sample_types'].get('markdown_chunk', 0)
        }
