#!/usr/bin/env python3
"""
Enhanced Dataset Preparation Script for Hyperswitch CPT
...
class EnhancedRustDatasetPreparator:
    ...
    def extract_quick_metadata(self, content: str) -> Dict:
        ...
        # Count key elements using simple regex
    ...
    def extract_public_functions(self, content: str, file_path: str, module: str) -> List[Dict]:
        ...
        # Match public functions with their doc comments
...
"""
            if mods:
                ...
                for mod in mods[:15]:  # Limit to avoid too long
                    text += f"pub mod {mod};\n"
                if len(mods) > 15:
            ...
            if uses:
                text += "// Public exports:\n"
                for use in uses[:15]:  # Limit to avoid too long
                    text += f"pub use {use};\n"
                if len(uses) > 15:
    ...
    def chunk_content(self, content: str, file_path: str, module: str) -> List[Dict]:
        ...
        # If fits in context window, return as single chunk
        ...
        # Need to chunk - use sliding window
        chunks = []
        lines = content.split('\n')
    ...
    def process_file(self, file_path: Path) -> Tuple[List[Dict], List[Dict]]:
        ...
        try:
            ...
            # Only extract granular data for non-test files with public APIs
    ...
    def save_dataset(self, file_samples: List[Dict], granular_samples: List[Dict]):
        ...
        # Save to single file
        output_path = self.output_dir / self.config['dataset']['data_file']
