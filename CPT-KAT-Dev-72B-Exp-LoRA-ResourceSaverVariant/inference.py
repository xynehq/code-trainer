...
# Model Configuration
BASE_MODEL_NAME = "Kwaipilot/KAT-Dev-72B-Exp"
...
def find_latest_checkpoint(output_base_dir=None):
    ...
    training_runs = [d for d in output_path.iterdir() 
                     if d.is_dir() and (d.name.startswith("hyperswitch-") and
                                        ("lora" in d.name.lower() or "qwen" in d.name.lower() or "kat-dev" in d.name.lower()))]
