...
def get_training_config():
    ...
    return {
        ...
        'run_name': f"hyperswitch-{model_config.get('name', 'qwen').split('/')[-1].lower()}-qlora",
        ...
        'wandb_tags': wandb_config.get('tags', ['rust', 'hyperswitch', 'qlora']),
        'wandb_notes': wandb_config.get('notes', 'QLoRA CPT training for Hyperswitch Rust codebase'),
        ...
        'gradient_checkpointing_use_reentrant': training_config.get('gradient_checkpointing_use_reentrant', False),
...
def initialize_wandb(config, timestamp, output_dir):
...
def main():
    ...
    wandb_enabled = initialize_wandb(TRAINING_CONFIG, timestamp, output_dir)
    ...
    # Load model WITHOUT HfDeepSpeedConfig (let Trainer handle it)
    logger.info("[3/5] Loading model with QLoRA (4-bit quantization)...")
    logger.info(f"  Model: {TRAINING_CONFIG['base_model']}")
    logger.info(f"  Quantization: {'4-bit NF4' if TRAINING_CONFIG['load_in_4bit'] else 'BF16 (full precision)'}")
    ...
    if TRAINING_CONFIG['load_in_4bit']:
        ...
        logger.info(f"    • Quant type: {TRAINING_CONFIG['bnb_4bit_quant_type']}")
        logger.info(f"    • Compute dtype: {TRAINING_CONFIG['bnb_4bit_compute_dtype']}")
        logger.info(f"    • Double quant: {TRAINING_CONFIG['bnb_4bit_use_double_quant']}")
        ...
        compute_dtype = compute_dtype_map.get(
            TRAINING_CONFIG['bnb_4bit_compute_dtype'], 
            torch.bfloat16
        )
        ...
        logger.info("  Expected memory: ~20-25 GB per GPU (vs ~65 GB in BF16)")
    
    logger.info("  Note: Model will be loaded to GPU 0 with quantization")
    logger.info("  DeepSpeed will handle distributed training (ZeRO-2 compatible with quantization)")
    logger.info("  Loading may take several minutes...")
    
    # Load model with low_cpu_mem_usage to reduce memory footprint
    # For QLoRA: Model loads to GPU 0 with device_map, then DeepSpeed manages distribution
    # Note: ZeRO-3 is incompatible with device_map, so we use ZeRO-2 for quantized models
    model = AutoModelForCausalLM.from_pretrained(
        TRAINING_CONFIG['base_model'],
        quantization_config=bnb_config,  # Add quantization config for QLoRA
        dtype=torch.bfloat16 if not TRAINING_CONFIG['load_in_4bit'] else None,  # Let quantization handle dtype
        trust_remote_code=True,
        attn_implementation="flash_attention_2",
        low_cpu_mem_usage=True,
        device_map="auto" if TRAINING_CONFIG['load_in_4bit'] else None,  # Auto device map for quantization
    )
    # Don't use use_cache - it's incompatible with gradient checkpointing
    model.config.use_cache = False
    
    logger.info(f"  Model loaded: {model.dtype if not TRAINING_CONFIG['load_in_4bit'] else '4-bit quantized'}")
    logger.info(f"  Parameters: {model.num_parameters() / 1e9:.2f}B")
    if TRAINING_CONFIG['load_in_4bit']:
        logger.info("  ✓ Model quantized to 4-bit (QLoRA)")
        logger.info("  ✓ Memory savings: ~4x reduction vs BF16")
    
    # Setup LoRA configuration (will be applied by SFTTrainer after DeepSpeed init)
    logger.info("\n[4/5] Setting up LoRA/QLoRA configuration...")
    ...
    logger.info(f"  {'QLoRA' if TRAINING_CONFIG['load_in_4bit'] else 'LoRA'} rank: {lora_config.r}, alpha: {lora_config.lora_alpha}")
    logger.info(f"  Target modules: {len(lora_config.target_modules)}")
    if TRAINING_CONFIG['load_in_4bit']:
        logger.info("  ✓ QLoRA: 4-bit base model + full-precision LoRA adapters")
    ...
    training_args = TrainingArguments(
        ...
        gradient_checkpointing_kwargs={"use_reentrant": TRAINING_CONFIG['gradient_checkpointing_use_reentrant']} if TRAINING_CONFIG['gradient_checkpointing'] else None,
        
        # DeepSpeed - ONLY for non-quantized models (incompatible with device_map used by quantization)
        # For quantized models: Don't use DeepSpeed, device_map handles distribution
        deepspeed="deepspeed_configs/zero3_h200.json" if not TRAINING_CONFIG['load_in_4bit'] else None,
    ...
    # Initialize trainer with TRL's SFTTrainer (with peft_config to apply PEFT after DeepSpeed init)
    ...
    trainer = SFTTrainer(
        ...
        peft_config=lora_config,  # Pass PEFT config - will apply after DeepSpeed init
    )
