# Hyperswitch CPT Training - KAT-Dev-72B-Exp-LoRA
...
- **Batch size**: 2 micro × 8 grad_accum × 4 GPUs = 64 effective
...
- **zero2.json**: ZeRO stage 2 (optimizer state sharding)
- **zero2_h200.json**: ZeRO stage 2 optimized for H200 GPUs
- **zero3.json**: ZeRO stage 3 (model + optimizer sharding)
- BF16 precision
- No CPU offloading
...
- Model: ~140GB (72B params in BF16)
- LoRA adapters: ~2.8GB (r=64, full LoRA - 7 modules)
- Optimizer: ~35GB (ZeRO-2 split across 4 GPUs)
- Activations: ~20GB (with gradient checkpointing)
- **Total: ~90GB / 141GB available per H200** (Comfortable fit with 4 GPUs!)

### Expected Training Time
- Total steps: ~1,410 (3 epochs)
- ~6-8 seconds per step (distributed across 4 GPUs)
- **Total: ~2.5-3 hours** on 4x H200
...
- **Eval loss**: TBD (experimental 72B run)
