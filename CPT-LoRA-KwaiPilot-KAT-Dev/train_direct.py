...
def main():
    ...
    # FlashAttention is CUDA-only; avoid on TPU/CPU
    ...
    # Compute trainable params before any potential FSDP wrapping to avoid shard-local counts
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())
    
    # Log LoRA details and trainable params before any wrapping
    ...
    # Note: Wrapping happens after PEFT so adapters are part of the graph.
    # Some PEFT versions may have issues with FSDP; if you hit errors,
    # consider using PEFT's native FSDP integration.
    ...
    training_args = TrainingArguments(
        ...
        fp16=False,  # TPU uses bf16; fp16 not supported
