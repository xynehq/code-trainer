...
class MoEFineTuner:
        ...
        try:
            ...
            for i, example in enumerate(tqdm(self.train_dataset, desc="Tokenizing train")):
                try:
                    ...
                    logger.warning(f"Failed to tokenize train sample {i}: {e}")
                    continue
            ...
            for i, example in enumerate(tqdm(self.eval_dataset, desc="Tokenizing eval")):
                try:
                    ...
                    logger.warning(f"Failed to tokenize eval sample {i}: {e}")
                    continue
            ...
            try:
                ...
                logger.warning(f"Failed to print sample: {e}")
    ...
    def create_trainer(self):
        ...
        training_args = TrainingArguments(
            ...
            save_only_model=True,  # Skip optimizer/scheduler state to avoid FSDP sync issues
        )
            ...
            def on_evaluate(self, args, state, control, metrics=None, **kwargs):
                if metrics is not None:
                    ...
                    try:
                        ...
                        if 'eval_token_accuracy' in metrics and metrics['eval_token_accuracy'] is not None:
                        ...
                        logger.warning(f"Failed to log token accuracy: {e}")
        ...
        class TrainingInfoCallback(TrainerCallback):
            ...
            def on_save(self, args, state, control, **kwargs):
                ...
                try:
                    ...
                    logger.warning(f"Failed to save training_info.json at checkpoint {state.global_step}: {e}")
                    # Don't crash the training - continue anyway
    ...
    def save_training_info(self, output_dir: str, metrics: Dict = None):
        ...
        try:
            ...
            try:
                ...
                pass
            model_name = model_config.get('_name_or_path', self.config.get('model', {}).get('name', 'Unknown'))
            ...
            if metrics:
                ...
                # Calculate perplexity if loss is available and not None
                try:
            ...
            training_info = {
                "training_metadata": {
                    ...
                    "final_epoch": self.config.get('training', {}).get('num_train_epochs', 1),
                ...
                "model_config": {
                    ...
                    "trainable_percentage": f"{100 * trainable_params / total_params:.4f}%" if total_params > 0 else "0%",
                },
                "lora_config": {
                ...
                "training_config": {
                    "num_epochs": self.config.get('training', {}).get('num_train_epochs', 1),
                    "per_device_train_batch_size": self.config.get('training', {}).get('per_device_train_batch_size', 1),
                    "per_device_eval_batch_size": self.config.get('training', {}).get('per_device_eval_batch_size', 1),
                    "gradient_accumulation_steps": self.config.get('training', {}).get('gradient_accumulation_steps', 1),
                    "effective_batch_size": (
                        self.config.get('training', {}).get('per_device_train_batch_size', 1) * 
                        self.config.get('training', {}).get('gradient_accumulation_steps', 1) * 
                        self.config.get('hardware', {}).get('num_gpus', 4)
                    ),
                    "learning_rate": self.config.get('training', {}).get('learning_rate', 0.0002),
                    "lr_scheduler_type": self.config.get('training', {}).get('lr_scheduler_type', 'cosine'),
                    "warmup_ratio": self.config.get('training', {}).get('warmup_ratio', 0.03),
                    ...
                    "optim": self.config.get('training', {}).get('optim', 'adafactor'),
                    "logging_steps": self.config.get('training', {}).get('logging_steps', 10),
                    "save_steps": self.config.get('training', {}).get('save_steps', 50),
                    "eval_steps": self.config.get('training', {}).get('eval_steps', 10),
                },
                "dataset_info": {
                    "train_samples": len(self.train_dataset) if hasattr(self, 'train_dataset') else 0,
                    "eval_samples": len(self.eval_dataset) if hasattr(self, 'eval_dataset') else 0,
                    "max_seq_length": self.config.get('model', {}).get('max_seq_length', 1024),
                    "data_source": self.config.get('repository', {}).get('path', 'Unknown'),
                },
            "hardware_config": {
                "num_gpus": self.config.get('hardware', {}).get('num_gpus', 4),
                "gpu_model": "NVIDIA H200",
                "gpu_memory_per_device_gb": self.config.get('hardware', {}).get('gpu_memory_per_device', 141),
            ...
            "moe_config": {
                "use_auxiliary_loss": self.config.get('moe', {}).get('use_auxiliary_loss', False),
                "auxiliary_loss_weight": self.config.get('moe', {}).get('auxiliary_loss_weight', 0.001),
                    "freeze_router": self.config.get('moe', {}).get('freeze_router', False),
                    "num_experts_per_token": self.config.get('moe', {}).get('num_experts_per_token', 2),
                    "monitor_expert_usage": self.config.get('moe', {}).get('monitor_expert_usage', False),
                },
                "performance_metrics": final_metrics,
                "framework_versions": framework_versions,
                "special_features": {
                    "flash_attention_2": self.config.get('model', {}).get('attn_implementation') == 'flash_attention_2',
                    "gradient_checkpointing": self.config.get('training', {}).get('gradient_checkpointing', False),
                    "bf16_training": self.config.get('training', {}).get('bf16', False),
                    "fsdp_training": True,
                    "attention_only_lora": True,
                    "frozen_experts": True,
                    "eval_accumulation": self.config.get('training', {}).get('eval_accumulation_steps') is not None,
                }
            }
            ...
            try:
                ...
                logger.error(f"Failed to write training_info.json: {e}")
            
            return training_info
            
        except Exception as e:
            logger.error(f"Error in save_training_info: {e}")
            logger.warning("Continuing without saving training_info.json")
            return {}
    ...
    def train(self):
        ...
        try:
            ...
            # Continue to save metrics anyway
        ...
        try:
            ...
            logger.warning(f"Failed to print training summary: {e}")
    ...
    def evaluate(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to save evaluation metrics: {e}")
    ...
    def merge_and_save(self):
        ...
        try:
            if not self.config.get('post_training', {}).get('merge_and_save', False):
